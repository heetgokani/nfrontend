...
const ProductSection = () => {
  ...
  const getImageUrl = (path) => {
    ...
    return `https://nbackend-31lg.onrender.com${path}`;
  };
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        const [res, attrRes] = await Promise.all([
          axios.get(`https://nbackend-31lg.onrender.com/api/products/${id}`),
      ...
      const fetchReviews = async () => {
        try {
          const res = await axios.get(
            `https://nbackend-31lg.onrender.com/api/reviews/${selectedVariant._id}`
          );
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await axios.post(
        "https://nbackend-31lg.onrender.com/api/cart/add",
        {
